...
The F1 spread across all 15 models is 0.39 (0.13--0.52), while the pass-rate spread on cpu\_ip is only 0.11 (most models pass exactly 1/9). On a single-axis benchmark the models would be indistinguishable, whereas on rtl-mosaic they separate cleanly.
...
We observed four recurring failure modes across the 240 calls. (1) \emph{Hallucinated picks}: the planner correctly tags a 32-bit ALU as \texttt{REUSE\_IP} but emits a search query the keyword router resolves to \texttt{priority\_encoder}, since the router does keyword overlap and a bad query produces a bad pick. (2) \emph{Over-decomposition}: weaker models split a five-line spec into six \texttt{GENERATE} subblocks and never reuse anything. (3) \emph{Under-decomposition}: GPT-5 returns one big \texttt{GENERATE} block on average ($1.0$ subblocks per problem), so the router never gets called, which explains why GPT-5 sits at the bottom of our leaderboard despite being a competent code model. (4) \emph{Kind-flip}: at temperature zero, some Anthropic models swing between \texttt{REUSE\_IP} and \texttt{GENERATE} on the same subblock across reruns. Claude Opus 4.7 went from F1=0.678 in an early 9-problem run to F1=0.375 in our final 16-problem run. Run-to-run variance is real even at deterministic settings, and multi-seed averaging is the obvious follow-up we did not yet do.

\subsection{The corpus-disambiguation bottleneck}

When we expanded the corpus from 20 to 30 IPs, mean F1 across the eval dropped. The drop comes from corpus disambiguation: adding \texttt{up\_down\_counter}, \texttt{johnson\_counter}, and \texttt{gray\_counter} alongside the existing \texttt{up\_counter} created near-duplicate keyword profiles, so a planner emitting \texttt{search\_query="counter"} resolves to whichever IP keyword matches first rather than the architecturally correct one. Replacing the keyword router with embedding similarity would directly address this, and we believe it would lift the mean F1 by 0.05--0.10. We list this as the highest-priority follow-up.
