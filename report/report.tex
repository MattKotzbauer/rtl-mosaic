...
\begin{abstract}
Existing LLM-for-Verilog benchmarks score models on from-scratch generation of RTL code. Real-world chip design tasks, however, tend to be more compositional, with modern SoCs running 60--70\% licensed IP by area. We built rtl-mosaic, a system-level harness that decomposes a Verilog spec into subblocks, routes reusable pieces to a curated 30-IP corpus over MCP, and stitches the result through Icarus Verilog testbenches. We then ran 15 frontier LLMs from four providers through the harness on 26 ChipBench problems (9 cpu\_ip + 17 self\_contain), scoring each against 26 hand-labeled gold entries. Across these runs, IP-reuse skill emerges as a distinct axis from scratch coding: routing F1 spreads from 0.19 to 0.46 across models while scratch pass rate on the same problems stays roughly flat. A reuse-aware metric reveals a model capability that scratch-only benchmarks are unable to capture.
\end{abstract}
...
Fig.~\ref{fig:leading}(b) plots each of the 15 LLMs as a dot, with the four colored dots representing best-of-provider headline models. The x-axis is scratch pass rate on the cpu\_ip set (single-shot Verilog through the ChipBench testbench) and the y-axis is routing F1 against our hand-labeled gold on the 26-problem eval set. The scatter shows no diagonal correlation between the two axes, meaning routing skill does not simply follow overall model strength.

\begin{table}[t]
  \caption{Headline routing F1, best-of-provider, 26-problem eval (9 cpu\_ip + 17 self\_contain). Single-seed column from the full 15-model sweep (23--26 successful calls per model depending on API availability), multi-seed column (n=3 seeds) from a focused re-run on the four headline models. Per-problem $\sigma$ quantifies run-to-run variance at temperature zero.}
  \label{tab:headline}
  \centering
  \footnotesize
...
\textbf{Matt Kotzbauer}: 30-IP corpus and self-tests, multi-provider runner, gold labels, eval metrics, figure pipeline, and the report. \textbf{Warren Zhu}: planner, integrator, ChipBench baseline runner, MCP IP-router, end-to-end harness CLI. 
